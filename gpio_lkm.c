...
bool gpio_unavailable = 0;
EXPORT_SYMBOL(gpio_unavailable);
...
struct gpio_lkm_dev
{
    ...
    enum state state;
    enum direction dir;
    /* false if gpio is used by some other kernel module */
    bool available;
};
...
struct gpio_lkm_dev *gpio_lkm_devp[MAX_GPIO_NUMBER];
/* */
static dev_t first;
...
extern int read_gpio_value(unsigned int gpio) {
    ...
    if (gpio_unavailable) {
        printk("[GPIL_LKM] Cannot command GPIO - unavailable");
	return -EPERM;
    }
    ...
    // printk("[GPIO LKM] Cannot read value");
...
static int __init gpio_lkm_init(void)
{
    ...
    for (i = 0; i < MAX_GPIO_NUMBER; i++)
    {
        if ( i == 4 || i == 17 || i == 18 || i == 27 ||
            ...
            i == 16 || i == 26 || i == 19)
        {
	    printk(KERN_DEBUG "[GPIO_LKM] Loading GPIO %d", i);
            /* allocate memory for sctuctures to contain each GPIO representation
            */
            gpio_lkm_devp[i] = kmalloc(sizeof(struct gpio_lkm_dev), GFP_KERNEL);

            if (!gpio_lkm_devp[i])
            {
                printk(KERN_DEBUG "[GPIO_LKM] Bad kmalloc\n");
		for (j = 0; j < i; ++j) 
		    if (gpio_lkm_devp[j]) gpio_free(j);
                return -ENOMEM;
            }
...
static void __exit gpio_lkm_exit(void)
{
    ...
    for (i = 0; i < MAX_GPIO_NUMBER; i++)
         ...
         */
	if (gpio_lkm_devp[i] != 0) kfree(gpio_lkm_devp[i]);
